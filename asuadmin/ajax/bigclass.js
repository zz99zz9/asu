...
$(".ssave").click(function(){
	BCName=jmyh($("#SBigClassName").val());
	SCName=jmyh($("#SmallClassName").val());
...
function XgSmallClass(BCName,BCID,SCName,SCID,SAction,SOID){
    ...
    function(data,status){
	  ...
	  if(status=="success"){
				...
				if (SAction=="add"){
					$("#alert span").html("二级分类 "+jmyh2(SCName)+" 添加成功。");
				}else if(SAction=="mod"){
					$("#alert span").html("二级分类 "+jmyh2(SCName)+" 修改成功。");
				}else if(SAction=="del"){
					$("#alert span").html("二级分类 "+jmyh2(SCName)+" 删除成功。");
				}
...
function XgBigClass(BCName,EBCName,Action,BCId,OID){
	  ...
	  if(status=="success"){
		var jsObjstr =JSON.parse(data);
		  ...
		  if (Action!="list"){
					...
					$("#alert strong").html("操作成功!");
				if (Action=="add"){
					$("#alert span").html("一级分类 "+jmyh2(BCName)+" 添加成功。");
				}else if(Action=="mod"){
					$("#alert span").html("一级分类 "+jmyh2(BCName)+" 修改成功。");
				}else if(Action=="del"){
					$("#alert span").html("一级分类 "+jmyh2(BCName)+" 删除成功。");
				}
		  }
	//刷新输出列表
	...
	str+='$("#BigClassName").val(jmyh2(bcname));';
	str+='$("#BAction").val("mod");';
...
str+='$("#xgbigclass").before("<p>是否确定要删除以下大类及相关产品信息吗？</p>");'
	...
	str+='$(".smod").click(function(){';
		str+='var bcname=jmyh2($(this).data("bcname"));';
		str+='var bcid=$(this).data("bcid");';
		str+='var scname=jmyh2($(this).data("scname"));';
